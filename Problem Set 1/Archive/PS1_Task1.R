...
equalWeightMonthlyReturns <- 
  Return.portfolio(na.omit(discrete_return),
...
corls <- cor(discrete_return)
s <- max(corls[corls!=max(corls)])

which(corls == s, arr.ind=TRUE)
which(corls ==min(corls), arr.ind = TRUE)

#Comment: When looking purely for diversification (regardless of any implies on return) the idea is to reduce the portfolio variance. Given the portfolio variance is defind by (copy 2.12 from lecture slides) the diversifcation benefit increases with decreasing correlation of two assets
# Therefore, to get the highest diversifcation benefit an investor should choose stocks SAP and E_ON as they have the lowest correlation out of the 10 stocks.
# The worst diversifcation benefit is achieved by only investing in a single stock (as correlation = 1). However, given two stocks need to be picked, the worst diversifcation effect is achieved with investing into RWE and E_ON given they have the highest correlation.
